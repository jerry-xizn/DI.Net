...
using DI.Framework.DataEncryption;
using DI.Framework.Exceptions;
using DI.Framework.JwtBearer;
...
namespace DI.Common.Utils
{
    public static class SecurityUtils
    {
        ...
        public static LoginUser GetLoginUser(HttpRequest request)
        {
            ...
            if (!string.IsNullOrEmpty(token))
            {
                try
                {
                    ...
                    // 解析对应的权限以及用户信息
                    string uuid = claims.Where(c => c.Type.Equals(DI.Data.Constants.LOGIN_USER_KEY)).First().Value;
        ...
        public static string GetToken(HttpRequest request)
        {
            ...
            if (!string.IsNullOrEmpty(token) && token.StartsWith(DI.Data.Constants.TOKEN_PREFIX))
            {
                token = token.Replace(DI.Data.Constants.TOKEN_PREFIX, "");
            }
            return token;
        }
