using DI.Common.Data;
using DI.Common.Enums;
using DI.Common.Files;
...
namespace DI.System.Controllers
{
    ...
    public class SysProfileController : ControllerBase
    {
        ...
        /// </summary>
        [HttpPut("")]
        [DI.System.Log(Title = "个人信息", BusinessType = BusinessType.UPDATE)]
        ...
        /// </summary>
        [HttpPut("updatePwd")]
        [DI.System.Log(Title = "重置密码", BusinessType = BusinessType.UPDATE)]
        ...
        /// </summary>
        [HttpPost("avatar")]
        [DI.System.Log(Title = "用户头像", BusinessType = BusinessType.UPDATE)]
        public async Task<object> UploadAvatar([Required] IFormFile file)
        {
            if (file != null)
            {
                LoginUser loginUser = SecurityUtils.GetLoginUser();
                string avatar = await FileUploadUtils.UploadAsync(file, DiApp.DiConfig.AvatarPath, MimeTypeUtils.IMAGE_EXTENSION);
