...
using DI.Framework;
using DI.Framework.DataValidation;
using DI.Framework.Exceptions;
using DI.Framework.Extensions;
using DI.Framework.Logging;
using DI.Framework.Templates;
using DI.Framework.UnifyResult;
...
public sealed class LoggingMonitorAttribute : Attribute, IAsyncActionFilter, IAsyncPageFilter, IOrderedFilter
{
    ...
    private async Task MonitorAsync(MethodInfo actionMethod, IDictionary<string, object> parameterValues, FilterContext context, dynamic next)
    {
        ...
        // 获取最终写入日志消息格式
        var finalMessage = GetJsonBehavior(JsonBehavior, monitorMethod) == DI.Framework.Logging.JsonBehavior.OnlyJson ? jsonString : monitorMessage;
